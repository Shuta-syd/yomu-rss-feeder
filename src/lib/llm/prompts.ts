...
export const STAGE1_SYSTEM_JP = `あなたはRSSフィード記事の要約アシスタントです。
入力は日本語記事です。タイトル翻訳は不要で、summaryとtagsのみJSON形式で返してください。
タグは以下から最大3つ選択: AI, LLM, Frontend, Backend, Database, DevOps, Cloud, Security, Mobile, Fintech, Startup, Business, Design, OSS, Other
...
export function isJapaneseTitle(title: string): boolean {
  const jp = (title.match(/[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF]/g) ?? []).length;
  const en = (title.match(/[A-Za-z]/g) ?? []).length;
  return jp > en;
}
